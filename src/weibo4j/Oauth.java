...
public class Oauth {
    ...
    public String getAccessTokenByCode(WeiboAuthInfo info, String code) throws WeiboException {
        AccessToken token = new AccessToken(Weibo.client.post(
                WeiboConfig.getValue("accessTokenURL"),
                new PostParameter[]{
                        new PostParameter("client_id", info.getAppKey()),
                        new PostParameter("client_secret", info.getAppSecret()),
                        new PostParameter("grant_type", "authorization_code"),
                        new PostParameter("code", code),
                        new PostParameter("redirect_uri", info.getRedirectURI())}, false));
    ...
    public String authorize(WeiboAuthInfo info, String response_type) throws WeiboException {
        return WeiboConfig.getValue("authorizeURL").trim() + "?client_id="
                + info.getAppKey() + "&redirect_uri=" + info.getRedirectURI()
                + "&response_type=" + response_type;
    }
}
